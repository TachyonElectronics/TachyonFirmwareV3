...
void loop()
{
	...
	switch (currentScreen)
	{
		...
		if (updateAmmoNextLoop)
		{
			...
			{
				disp.fillRect(digitPos.x, digitPos.y, digitPos.spacingx *3 - 4, 60, settings.bgColor);
			}
			updateAmmoBar();
			...
			for (char i = 0; i < 3; i++)
			{
				uint8_t *digit = (i == 2) && ammo < 0 ? (uint8_t*)C_Ldash : C_Ldigits[digits[3 - i]];
				disp.drawBitmap(digitPos.x + digitPos.spacingx * (2 - i), digitPos.y, /*((i == digitCount - 1) && ammo < 0) ? dash : */ digit, ammoColor, settings.bgColor);
			}
			#endif
...
void displayMainScreen()
{
	...
	disp.fillScreen(settings.bgColor);
	disp.drawBitmap(0,0,MainBackground,settings.uiColor, settings.bgColor);
	// disp.setTextColor(settings.uiColor,settings.bgColor);
	disp.drawBitmap(7, 67, IconPreset, settings.uiColor, settings.bgColor);
	disp.drawBitmap(44, 117, IconBattery, settings.uiColor, settings.bgColor);
	...
	disp.drawBitmap(HA_X, HA_Y,IconUP, settings.statusColors[2],settings.bgColor);
...
void updateAmmoBar()
{
	...
	{
		size =
		ammo > 0 ? map(ammo, 0, settings.presets[currentPreset], 0, AMMOBAR_WIDTH) : 0;
...
void drawAmmoBar(uint8_t size)
{
	if (size == 0)
	{
		disp.fillRect(AMMOBAR_X, AMMOBAR_Y, AMMOBAR_WIDTH, AMMOBAR_HEIGHT, settings.bgColor);
		return;
	}
	int16_t crop = AMMOBAR_WIDTH - size;
	if(settings.selectedAmmoBar)
	disp.drawBitmap(AMMOBAR_X,AMMOBAR_Y, AmmoBars[settings.selectedAmmoBar - 1], ammoColor,	settings.bgColor, settings.ammoBarDir ? -crop : crop, 0);
	else
	disp.fillCroppedRect(AMMOBAR_X,AMMOBAR_Y,AMMOBAR_WIDTH,AMMOBAR_HEIGHT ,ammoColor,settings.bgColor,settings.ammoBarDir ? -crop : crop, 0);
}
...
{
	// disp.setTextColor(settings.uiColor, settings.bgColor);
	disp.setCursor(12, 67);
	disp.setTextSize(2);
	disp.print(changingPreset ? changedPreset + 1 : currentPreset + 1);
	disp.setTextSize(1);

	disp.drawBitmap(12, 82, IconSelectPreset, changingPreset ? settings.uiColor : settings.bgColor, settings.bgColor);
}
...
void updateHAssist(){
	...
	if(settings.hAssistEnabled & 1 && newColor != lastHAssistColor){
		disp.drawBitmap(HA_X, HA_Y, IconUP,newColor ?  settings.statusColors[0]: settings.statusColors[2],settings.bgColor);
		lastHAssistColor = newColor;
	}
}
uint8_t currentCompassDirection = 0xff;
void updateCompass()
{
	...
	uint8_t dir = (360-(bearing+22)) / 45;
	dir &= 7; //Clip to 8 possible directions 
	if(currentCompassDirection != dir)
	{
		disp.drawBitmap(54, 7, CompassIcons[dir], settings.uiColor, settings.bgColor);
		currentCompassDirection = dir;
	}
}
#pragma endregion
//=============================================================
...
void factoryReset()
{
	...
	factorySettings.bgColor = FACT_BGCOLOR;
