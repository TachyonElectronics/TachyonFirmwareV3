...
void Adafruit_ST7735::drawBitmap(uint8_t x_in, uint8_t y_in, const uint8_t bitmap[], uint16_t color, uint16_t bg, int16_t cropX, int16_t cropY)
{
	...
	uint16_t size = w * h;
	uint8_t byte, xPos = 0, yPos = 0, bitmask = 0;
	uint16_t bytePos = 2;
	CS_LOW();
	
	for(uint16_t i = 0; i < size; i++)
	{
		if(!bitmask) //load new byte every 8 bits
		{
			byte = pgm_read_byte(&bitmap[bytePos]);
			bytePos++;
			bitmask = 0x80;
		}
		
		bool b = (byte & bitmask) && !(xPos < cropLeft || xPos >= w-cropRight || yPos < cropTop || yPos >= h-cropBottom);

		//Bitbang assembly version
		/*asm volatile(
		"sbi 0x05, 3\n\t" //set D/C bit to 1 (9th bit in the transferred 'byte')
		"sbi 0x05, 5\n\t" //pulse clock
		"cbi 0x05, 5"
		);
		register uint8_t buffer = b? cH : bgH;
		register uint8_t spi_bitPos;
		for(spi_bitPos = 0x80; spi_bitPos; spi_bitPos >>= 1)
		{
		asm volatile(
		"sbi 0x05, 3\n\t" //Set MOSI to 1 by default
		"cpse %0, %1\n\t"   //skip next statement if bit is 1...
		"cbi 0x05, 3\n\t" //...otherwise set MOSI to 0
		"sbi 0x05, 5\n\t" //pulse clock
		"cbi 0x05, 5"
		: :"r" (spi_bitPos), "r" (buffer & spi_bitPos)
		);
		}
		asm volatile(
		"sbi 0x05, 3\n\t" //set D/C bit to 1 (9th bit in the transferred 'byte')
		...
		DC_HIGH(); //transmit 9th bit (data/command)
		SPI.transfer(b? cH : bgH); //Send data through SPI


		DC_HIGH();		
		SPDR = (b? cL : bgL);

		//Increment bitPos with modulo 7 (will overflow over 7)
		bitmask >>= 1;
